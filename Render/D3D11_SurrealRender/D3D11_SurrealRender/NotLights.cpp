...
XMFLOAT4 DirectionalLight::AddRotationInput(float X, float Y, float Z, bool bIgnoreDeltaTime)
{
	...
	XMVECTOR RotateIt = { (X * Multiplier), (Y * Multiplier), (Z * Multiplier) };
	XMMATRIX RotationMatrix = XMMatrixRotationRollPitchYawFromVector(RotateIt);
	XMVECTOR Final = XMVector3Transform(XMLoadFloat4(&Direction), RotationMatrix);

	XMStoreFloat4(&Direction, Final);
...
// Initialize this light for use in the game world. Takes in a direction, color, and intensity. It also needs the DisplayAgent whose world list should have this light.
void SpotLight::CreateLight(const char* DebugName, XMFLOAT4 color, float intensity)
{
	Name = DebugName;
	Color = color;
	Intensity = intensity;
...
// Rotate this object in a 3D world. Return the object's new World Matrix after rotation. Set bBaseOnWorld to false if you want the rotation to be based on the current camera rotation rather than the world matrix. Set ignore DeltaTime to true to force the exact rotation and ignore the tick update.
XMFLOAT4X4 SpotLight::AddRotationInput(float Pitch, float Yaw, float Roll, bool bIgnoreDeltaTime)
{
	float Multiplier = 1.0f;
	if (bIgnoreDeltaTime)
	{
...
XMFLOAT4X4 SpotLight::AddMovementInput(float x, float y, float z, bool bIgnoreDeltaTime)
{
	float Multiplier = 1.0f;
	if (bIgnoreDeltaTime)
	{
